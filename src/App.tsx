...
function TreatmentSummary({teeth,milkTeeth,onTeethChange,onMilkChange,patient,onChange}){
  ...
  const toggleTreated=(item)=>{
    ...
    if(item.milk) onMilkChange(newMilk);
    else onTeethChange(newTeeth);

    // Si se marca como TRATADO → generar/actualizar entrada en evolución
    if(!item.treated && patient && onChange){
      const todayStr=new Date().toISOString().slice(0,10);
      const evolution=[...(patient.evolution||[])];

      // Buscar si ya existe una entrada automática de hoy
      const autoId=`auto-tratamiento-${todayStr}`;
      const existIdx=evolution.findIndex(e=>e.id===autoId);

      // Detectar otras piezas ya tratadas hoy en la misma entrada
      const alreadyInNote=existIdx>=0?(evolution[existIdx].tooth||""):"";
      const piezasHoy=alreadyInNote
        ?alreadyInNote.split(",").map(s=>s.trim()).filter(Boolean)
        :[];
      ...
      if(existIdx>=0){
        // Actualizar entrada existente de hoy
        evolution[existIdx]={...evolution[existIdx],tooth:toothStr,note:noteStr};
      } else {
        // Crear nueva entrada automática
        evolution.unshift({
          id:autoId,
          date:todayStr,
          tooth:toothStr,
          treatment:"Tratamiento odontológico",
          note:noteStr,
          professional:"",
      ...
      onChange({...patient,evolution,updatedAt:new Date().toISOString()});
    }

    // Si se DESMARCA → quitar la pieza de la entrada automática de hoy
    if(item.treated && patient && onChange){
      const todayStr=new Date().toISOString().slice(0,10);
      const autoId=`auto-tratamiento-${todayStr}`;
      const evolution=[...(patient.evolution||[])];
      ...
      if(existIdx>=0){
        ...
        if(piezas.length===0){
          // No quedan piezas → eliminar la entrada
          evolution.splice(existIdx,1);
        } else {
          const toothStr=piezas.join(", ");
          evolution[existIdx]={...evolution[existIdx],tooth:toothStr,note:`Tratamiento realizado — Piezas: ${toothStr}`};
        }
        onChange({...patient,evolution,updatedAt:new Date().toISOString()});
      }
    }
...
function Dashboard({currentProf,patients,allPatients,onSelectPatient,onCreatePendingPatient}){
  ...
  const addQuickAppt=async()=>{
    ...
    if(quickMode==="new"){
      ...
      // Crear paciente "pendiente" — ficha mínima
    ...
    const newAppt={id:Date.now().toString(),patientId,date:selectedDay,
      ...
      createdAt:new Date().toISOString()};
    const updated=[...appointments,newAppt].sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time));
    await saveAppts(updated);
      ...
      <div style={{marginBottom:20}}>
        ...
        {showQuickForm&&(
          <div style={{backgroundColor:"#f8fafc",borderRadius:12,padding:16,marginBottom:12,border:"1px solid #e2e8f0"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setShowQuickForm(false)} style={btnSecondary}>Cancelar</button>
